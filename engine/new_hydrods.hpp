...
public:
    ...
    NewHydroDS(const NewHydroDS&) = delete;
    NewHydroDS& operator=(const NewHydroDS&) = delete;

    // Movable
    NewHydroDS(NewHydroDS&& other) noexcept
    ...
    NewHydroDS& operator=(NewHydroDS&& other) noexcept {
        if (this != &other) {
            for (auto* b : buckets_) free_bucket(b);
            buckets_ = std::move(other.buckets_);
            bucket_max_ = std::move(other.bucket_max_);
